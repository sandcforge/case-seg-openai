...
class TestMessageCoverage(unittest.TestCase):
    ...
    def test_message_completeness(self):
        ...
        for case in self.global_cases:
            msg_list = case.get("msg_index_list", [])
    ...
    def test_case_id_format(self):
        """Validate case_id format consistency (should be chunk_id#case_id)"""
        ...
        for case in self.global_cases:
            ...
            if not pattern.match(case_id):
                invalid_formats.append(case_id)
        
        self.assertEqual(
            invalid_formats,
            [],
            f"Invalid case_id formats found: {invalid_formats}"
        )
    ...
    def test_chunk_continuity(self):
        ...
        for case in self.global_cases:
            ...
            if '#' in case_id_str:
                chunk_id, case_id = case_id_str.split('#')
    ...
    def test_case_structure(self):
        ...
        required_fields = ["case_id", "msg_index_list", "summary", "status"]
        ...
        for i, case in enumerate(self.global_cases):
            ...
            # Validate msg_index_list is non-empty list
            msg_list = case.get("msg_index_list", [])
            self.assertIsInstance(msg_list, list, f"Case {i} msg_index_list should be a list")
            self.assertGreater(len(msg_list), 0, f"Case {i} msg_index_list should not be empty")
            
            # Validate all msg_index_list items are integers
            for msg_id in msg_list:
